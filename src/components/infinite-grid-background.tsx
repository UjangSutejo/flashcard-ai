...
export const InfiniteGrid = () => {
    ...
    // Track mouse position globally via window — works even when content layers are on top
    ...
    return (
        <div
            ref={containerRef}
            className={cn(
                "fixed inset-0 w-full h-full -z-10 flex flex-col items-center justify-center overflow-hidden bg-background"
            )}
        >
            ...
            {/* Optional: Grid Density Control Slider (can be enabled if needed) */}
            {/* 
            <div className="absolute bottom-4 right-4 z-30 pointer-events-auto">
                <input
                    ...
                    className="w-24 h-1.5 bg-secondary rounded-lg appearance-none cursor-pointer accent-primary opacity-30 hover:opacity-100 transition-opacity"
                />
            </div>
            */}
        </div>
